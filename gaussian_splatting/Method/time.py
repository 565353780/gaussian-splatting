...
def getLatestFolderName(base_folder_name, folder_root_path):
    ...
    for folder_name in folder_name_list:
        ...
        if max_folder_date is None:
            max_folder_date = int(folder_date)
            max_folder_h = int(folder_h)
            max_folder_m = int(folder_m)
            max_folder_s = int(folder_s)
            continue

        if int(folder_date) > max_folder_date:
            max_folder_date = int(folder_date)
            max_folder_h = int(folder_h)
            max_folder_m = int(folder_m)
            max_folder_s = int(folder_s)
